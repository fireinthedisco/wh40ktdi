...
VertexShader =
{
	...
	[[
		...
		{
			VS_OUTPUT Out;
			Out.vPosition  	= mul( ViewProjectionMatrix, float4( v.vPosition.x, -20.0f, v.vPosition.y, 1.0f ) );
			Out.vUV			= v.vUV;
...
PixelShader =
{
	...
	[[
		float4 main( VS_OUTPUT v ) : PDX_COLOR
		{
			...
			float vAlphaMin = 0.2f + 0.5f * vCamDistFactor;
			...
			float vAlphaEdge = saturate( (vDist-vMid + vEdgeWidth)*vEdgeSharpness ) * 0.3f;
			//vAlphaFill is the soft gradient inside the blobs
			float vAlphaFill = max( vAlphaMin, saturate( vMid + (vDist-0.25f + vEdgeWidth*1.0f)*2.0f ) * 0.85f );
